...
#from gfn.utils.modules_ENN import MLP_ENN
#from gfn.utils.modules_ENN_IMP import MLP_ENN
from algos.ENN_GFN_ENHANCED import MLP_ENN
...
def plot_results(env, gflownet):
    fig = plt.figure(figsize=(15, 5))

    # Define layout: [True, Colorbar, Learned, (optional empty space)]
    gs = GridSpec(1, 4, width_ratios=[1, 0.05, 1, 0.2])  # Colorbar is now at index 1

    ax1 = fig.add_subplot(gs[0])  # True Distribution
    cax = fig.add_subplot(gs[1])  # Colorbar
    ax2 = fig.add_subplot(gs[2])  # Learned Distribution
    ...
    # Fix color scale based on true distribution only
    vmin = true_dist.min()
    vmax = true_dist.max()
    ...
    # Colorbar next to True Distribution
    plt.colorbar(im1, cax=cax)
...
def main(args):  # noqa: C901
    ...
    for iteration in pbar:
        ...
        fixed_idx = torch.randint(0, policy.index_dim, (1,)).item()
        policy.set_index(fixed_idx)  # Lock the head index
        ...
        if my_rank == 0:
            ...
            if log_this_iter:
                ...
                if iteration % 100 == 0:
                    l1_distance_enn_imp.append((iteration, validation_info["l1_dist"],  validation_info["n_modes_found"] ))
    ...
    if args.plot:
        # Create figure with 3 subplots with proper spacing.
        plot_results(env, gflownet)
    import csv
    with open(".csv", "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["iteration", "l1_distance","n_modes_found" ])
        writer.writerows(l1_distance_enn_imp)
    try:
